...
# Aliases avoid Next.js reserving exact path /api/chat in standalone mode
@router.post("/create", response_model=ChatResponse)
def create_chat_alias(
    *,
    db: Session = Depends(get_db),
    chat_in: ChatCreate,
    current_user: User = Depends(get_current_user),
) -> Any:
    return create_chat(db=db, chat_in=chat_in, current_user=current_user)
...
def list_chats_alias(
    ...
    return get_chats(db=db, current_user=current_user, skip=skip, limit=limit)
